...
#' sun  = Sunshine duration  (0-24 hours)
#' cl  = Cloudiness         (0-1)
#' cl is optional for use with sun, if unavailble cl is estimated
...
#browser()
  ...
  } else if (is.null(vp) &&  !is.null(indat$dp)) {
    # estimate vp from dp
    rh <- dewpoint_to_humidity(dp, tm)
    vp <- (rh / 100) * vps
  ...
  # check for pressence of sun and cl
  if (is.null(indat$sun)) {
    ss_pres <- FALSE
  } else {
    ss_pres <- TRUE
  }
  ...
  # solar constant
  Gsc <- 0.0820
  ktn <- indat$tmin + 273.16
  ktx <- indat$tmax + 273.16
  ...
  if(ss_pres) {
    ...
    if (!cld_pres) {
      ...
      cl[dyln_zero] <- 1.0
      cl[dyln_nonzero] <- 1.0 - indat$sun[dyln_nonzero] / dyln[dyln_nonzero]
    }
    ssf <- 1.0 - cl

    rns <- 0.77 * (0.25 + (0.50 * (ssf))) * dter
    rnl <- sigma * (0.9 * (ssf) + 0.1) * (0.34-0.14*sqrt(avp)) * (ktn^4.0 + ktx^4.0)
  } else {
    ...
    rnl <- sigma * (0.34 - 0.14 * sqrt(avp)) * (ktx^4 + ktn^4)/2 * (1.35 * rs_div_rso - 0.35)
    rnl[rnl < 0] <- 0
  }
  # Net radiation
  rnet <- rns - rnl
  vptn <- 0.1 * vapour_pressure(indat$tmin)
  vptx <- 0.1 * vapour_pressure(indat$tmax)
  vpd <- ((vptx + vptn) / 2.0) - avp
...
dewpoint_to_humidity <- function (dp = NA, t = NA)
{
  ...
  if (length(dp[dp > t & !is.na(dp) & !is.na(t)]) > 0) {
    dp[dp > t] <- NA
    warning("For some observations, dew point temperature was higher than temperature.
            Since dew point temperature cannot be higher than air temperature, relative humidty for these observations was set to 'NA'.")
  }
